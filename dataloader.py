...
class DatasetGenerator(Sequence):
    ...
    def crop_input(self, img, msk):
        ...
        # Do we randomize?
        is_random = self.augment and np.random.rand() > 0.75
    ...
    def generate_batch_from_files(self):
        ...
        while True:
            ...
            """
            NUM_QUEUED_IMAGES = 1 + \
                self.batch_size // self.num_slices_per_scan  # Get enough for full batch + 1
            ...
            if (idy + self.batch_size) < num_slices:  # We have enough slices for batch
                img_batch, label_batch = img[:, :, idy:idy +
                                             self.batch_size], label[:, :, idy:idy+self.batch_size]

            else:  # We need to pad the batch with slices

                # Get remaining slices
                img_batch, label_batch = img[:, :, -
                                             self.batch_size:], label[:, :, -self.batch_size:]
    ...
    def plot_samples(self):
        ...
        plt.title("MRI, Slice #{}".format(slice_num), fontname='Monaco')

        plt.subplot(2, 2, 2)
        plt.imshow(label[slice_num, :, :, 0])
        plt.title("Tumor, Slice #{}".format(slice_num), fontname='Monaco')
